...
const double lines[32][5] = {
                            {1, 100.0, 100.0, 150.0, 0}, //leg t1L1 (1)
                            {1, 100.0, 70.0, 150.0, 300},
                            {1, 100.0, 40.0, 150.0, 300},
                            {1, 120.0, 70.0, 100.0, 300},
                            {1, 100.0, 100.0, 150.0, 300},
                            
                            {1, 96.0, 40.0,  150.0, 0},  //(2)
                            {1, 61.0, 61.0, 120.0, 300},
                            {1, 40.0, 96.0, 150.0, 300}, 
                            {1, 61.0, 61.0, 150.0, 300},
                            {1, 96.0, 40.0,  150.0, 300},                          
                            
                          

                            {1, 40.0, 100.0, 150.0, 0},//leg t1L1 (3)
                            {1, 70.0, 120.0, 150.0, 300},
                            {1, 100.0, 100.0, 150.0, 300}, 
                            {1, 70.0, 100.0, 100.0, 300},
                            {1, 40.0, 100.0, 150.0, 300},
                            
                            


                            {1, 100.0, 100.0, 150.0, 0}, //leg t1L1 (4)
                            {1, 120.0, 70.0, 100.0, 300},
                            {1, 100.0, 40.0, 150.0, 300},
                            {1, 100.0, 70.0, 150.0, 300},
                            {1, 100.0, 100.0, 150.0, 300},
                            ...
                            {1, 96.0, 40.0,  150.0, 0}, //leg t1L3 (5)
                            {1, 61.0, 61.0, 150.0, 300},
                            {1, 40.0, 96.0, 150.0, 300}, 
                            {1, 61.0, 61.0, 120.0, 300}, 
                            {1, 96.0, 40.0,  150.0, 300},                         
                            ...
                            {1, 40.0, 100.0, 150.0, 0},//leg t1L1 (6)
                            {1, 70.0, 100.0, 100.0, 300},
                            {1, 100.0, 100.0, 150.0, 300},
                            {1, 70.0, 120.0, 150.0, 300},
                            {1, 40.0, 100.0, 150.0, 300},
...
double J1A = atan((YA-28) / (XA-28)) * (180 / PI);
  double HA = sqrt(((YA-28) * (YA-28)) + ((XA-28) * (XA-28)));
  double LA = sqrt((ZA * ZA) + ((HA) * (HA)));
  double J3A = acos(   (51050-(LA * LA))   /   (38950)   ) * (180 / PI);
  double BA = acos(   ((LA * LA) - 33000)   /   (190 * LA)   ) * (180 / PI);
  double AA = atan(ZA / (HA)) * (180 / PI);  // BECAUSE Z REST IS NEGATIVE, THIS RETURNS A NEGATIVE VALUE
  double J2A = (BA - AA);

  double J1B = atan((YB-28) / (XB-28)) * (180 / PI);
  double HB = sqrt(((YB-28) * (YB-28)) + ((XB-28) * (XB-28)));
  double LB = sqrt((ZB * ZB) + ((HB) * (HB)));
  double J3B = acos(   (51050-(LB * LB))   /   (38950)   ) * (180 / PI);
  double BB = acos(   ((LB * LB) - 33000)   /   (190 * LB)   ) * (180 / PI);
  double AB = atan(ZB / (HB)) * (180 / PI); // BECAUSE Z REST IS NEGATIVE, THIS RETURNS A NEGATIVE VALUE
  double J2B = (BB - AB);

  double J1C = atan((YC-28) / (XC-28)) * (180 / PI);
  double HC = sqrt(((YC-28) * (YC-28)) + ((XC-28) * (XC-28)));
  double LC = sqrt((ZC * ZC) + ((HC) * (HC)));
  double J3C = acos(   (51050-(LC * LC))   /   (38950)   ) * (180 / PI);
  double BC = acos(   ((LC * LC) - 33000)   /   (190 * LC)   ) * (180 / PI);
  double AC = atan(ZC / (HC)) * (180 / PI);  // BECAUSE Z REST IS NEGATIVE, THIS RETURNS A NEGATIVE VALUE
  double J2C = (BC - AC);

  double J1D = atan((YD-28) / (XD-28)) * (180 / PI);
  double HD = sqrt(((YD-28) * (YD-28)) + ((XD-28) * (XD-28)));
  double LD = sqrt((ZD * ZD) + ((HD) * (HD)));
  double J3D = acos(   (51050-(LD * LD))   /   (38950)   ) * (180 / PI);
  double BD = acos(   ((LD * LD) - 33000)   /   (190 * LD)   ) * (180 / PI);
  double AD = atan(ZD / (HD)) * (180 / PI);  // BECAUSE Z REST IS NEGATIVE, THIS RETURNS A NEGATIVE VALUE
  double J2D = (BD - AD);

  double J1E = atan((YE-28) / (XE-28)) * (180 / PI);
  double HE = sqrt(((YE-28) * (YE-28)) + ((XE-28) * (XE-28)));
  double LE = sqrt((ZE * ZE) + ((HE) * (HE)));
  double J3E = acos(   (51050-(LE * LE))   /   (38950)   ) * (180 / PI);
  double BE = acos(   ((LE * LE) - 33000)   /   (190 * LE)   ) * (180 / PI);
  double AE = atan(ZE / (HE)) * (180 / PI);  // BECAUSE Z REST IS NEGATIVE, THIS RETURNS A NEGATIVE VALUE
  double J2E = (BE - AE);

  double J1F = atan((YF-28) / (XF-28)) * (180 / PI);
  double HF = sqrt(((YF-28) * (YF-28)) + ((XF-28) * (XF-28)));
  double LF = sqrt((ZF * ZF) + ((HF) * (HF)));
  double J3F = acos(   (51050-(LF * LF))   /   (38950)   ) * (180 / PI);
  double BF = acos(   ((LF * LF) - 33000)   /   (190 * LF)   ) * (180 / PI);
  double AF = atan(ZF / (HF)) * (180 / PI);  // BECAUSE Z REST IS NEGATIVE, THIS RETURNS A NEGATIVE VALUE
  double J2F = (BF - AF);
